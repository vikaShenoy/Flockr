...
public class ChatRepository {
   ...
   */
  public CompletionStage<ChatGroup> createChat(ChatGroup chatGroup) {
    return supplyAsync(
        () -> {
          chatGroup.insert();
          return chatGroup;
   ...
   */
  public CompletionStage<ChatGroup> modifyChat(ChatGroup chatGroup) {
    return supplyAsync(
        () -> {
          chatGroup.save();
          return chatGroup;
  ...
  public CompletionStage<List<ChatGroup>> getChatsByUserId(int userId) {
    return supplyAsync(
        () -> ChatGroup.find.query().fetch("users").where().eq("users.userId", userId).findList(),
        executionContext);
  }
  ...
  /**
   * Gets a message object using the id of the message.
   *
   * @param messageId the id of the message
   * @return the message object wrapped in an optional object.
   */
  public CompletionStage<Optional<Message>> getMessageById(int messageId) {
    return supplyAsync(
        () -> Message.find.query().where().eq("messageId", messageId).findOneOrEmpty(),
        executionContext);
  }
